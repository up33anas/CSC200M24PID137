...
// Separate component for a single foundation pile
function FoundationPile({ pile, index, viewModel }) {
  const symbols = ["♥", "♦", "♣", "♠"];
  const topCard = pile[pile.length - 1];

  // Hook is now called at the top level of this component
  const [{ isOver }, drop] = useDrop(() => ({
    accept: "CARD",
    drop: (item) => {
      try {
...
// Main FoundationView maps piles to FoundationPile components
export default function FoundationView({ foundation, viewModel }) {
  return (
    <div className="flex gap-6">
      {foundation.piles.map((pile, index) => (
